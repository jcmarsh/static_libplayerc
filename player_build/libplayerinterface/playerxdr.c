...
#include </home/jcmarsh/research/player/build/libplayerinterface/playerxdr.h>
#include <string.h>
...
int xdr_player_simulation_time_req_t (XDR* xdrs, player_simulation_time_req_t * msg)
{   if(xdr_longlong_t(xdrs,&msg->frank) != 1)
    return(0);
  return(1);
}
int 
player_simulation_time_req_pack(void* buf, size_t buflen, player_simulation_time_req_t * msg, int op)
{
  XDR xdrs;
  int len;
  if(!buflen)
    return 0;
  xdrmem_create(&xdrs, buf, buflen, op);
  if(xdr_player_simulation_time_req_t(&xdrs,msg) != 1)
    return(-1);
  if(op == PLAYERXDR_ENCODE)
    len = xdr_getpos(&xdrs);
  else
    len = sizeof(player_simulation_time_req_t);
  xdr_destroy(&xdrs);
  return(len);
} 
unsigned int player_simulation_time_req_t_copy(player_simulation_time_req_t *dest, const player_simulation_time_req_t *src)
{
  if (dest == NULL || src == NULL)
    return 0;
  memcpy(dest,src,sizeof(player_simulation_time_req_t));
  return sizeof(player_simulation_time_req_t);
} 
void player_simulation_time_req_t_cleanup(const player_simulation_time_req_t *msg)
{
} 
player_simulation_time_req_t * player_simulation_time_req_t_clone(const player_simulation_time_req_t *msg)
{      
  player_simulation_time_req_t * clone = malloc(sizeof(player_simulation_time_req_t));
  if (clone)
    player_simulation_time_req_t_copy(clone,msg);
  return clone;
}
void player_simulation_time_req_t_free(player_simulation_time_req_t *msg)
{      
  player_simulation_time_req_t_cleanup(msg);
  free(msg);
}
unsigned int player_simulation_time_req_t_sizeof(player_simulation_time_req_t *msg)
{
  return sizeof(player_simulation_time_req_t);
} 
...
int xdr_player_smrtln_param_cmd_t (XDR* xdrs, player_smrtln_param_cmd_t * msg)
{   if(xdr_int(xdrs,&msg->param_index) != 1)
    return(0);
  if(xdr_double(xdrs,&msg->param_value) != 1)
    return(0);
  return(1);
}
int 
player_smrtln_param_cmd_pack(void* buf, size_t buflen, player_smrtln_param_cmd_t * msg, int op)
{
  XDR xdrs;
  int len;
  if(!buflen)
    return 0;
  xdrmem_create(&xdrs, buf, buflen, op);
  if(xdr_player_smrtln_param_cmd_t(&xdrs,msg) != 1)
    return(-1);
  if(op == PLAYERXDR_ENCODE)
    len = xdr_getpos(&xdrs);
  else
    len = sizeof(player_smrtln_param_cmd_t);
  xdr_destroy(&xdrs);
  return(len);
} 
unsigned int player_smrtln_param_cmd_t_copy(player_smrtln_param_cmd_t *dest, const player_smrtln_param_cmd_t *src)
{
  if (dest == NULL || src == NULL)
    return 0;
  memcpy(dest,src,sizeof(player_smrtln_param_cmd_t));
  return sizeof(player_smrtln_param_cmd_t);
} 
void player_smrtln_param_cmd_t_cleanup(const player_smrtln_param_cmd_t *msg)
{
} 
player_smrtln_param_cmd_t * player_smrtln_param_cmd_t_clone(const player_smrtln_param_cmd_t *msg)
{      
  player_smrtln_param_cmd_t * clone = malloc(sizeof(player_smrtln_param_cmd_t));
  if (clone)
    player_smrtln_param_cmd_t_copy(clone,msg);
  return clone;
}
void player_smrtln_param_cmd_t_free(player_smrtln_param_cmd_t *msg)
{      
  player_smrtln_param_cmd_t_cleanup(msg);
  free(msg);
}
unsigned int player_smrtln_param_cmd_t_sizeof(player_smrtln_param_cmd_t *msg)
{
  return sizeof(player_smrtln_param_cmd_t);
} 

int xdr_player_smrtln_supsensor_cmd_t (XDR* xdrs, player_smrtln_supsensor_cmd_t * msg)
{   if(xdr_int(xdrs,&msg->sensor_index) != 1)
    return(0);
  if(xdr_int(xdrs,&msg->state) != 1)
    return(0);
  return(1);
}
int 
player_smrtln_supsensor_cmd_pack(void* buf, size_t buflen, player_smrtln_supsensor_cmd_t * msg, int op)
{
  XDR xdrs;
  int len;
  if(!buflen)
    return 0;
  xdrmem_create(&xdrs, buf, buflen, op);
  if(xdr_player_smrtln_supsensor_cmd_t(&xdrs,msg) != 1)
    return(-1);
  if(op == PLAYERXDR_ENCODE)
    len = xdr_getpos(&xdrs);
  else
    len = sizeof(player_smrtln_supsensor_cmd_t);
  xdr_destroy(&xdrs);
  return(len);
} 
unsigned int player_smrtln_supsensor_cmd_t_copy(player_smrtln_supsensor_cmd_t *dest, const player_smrtln_supsensor_cmd_t *src)
{
  if (dest == NULL || src == NULL)
    return 0;
  memcpy(dest,src,sizeof(player_smrtln_supsensor_cmd_t));
  return sizeof(player_smrtln_supsensor_cmd_t);
} 
void player_smrtln_supsensor_cmd_t_cleanup(const player_smrtln_supsensor_cmd_t *msg)
{
} 
player_smrtln_supsensor_cmd_t * player_smrtln_supsensor_cmd_t_clone(const player_smrtln_supsensor_cmd_t *msg)
{      
  player_smrtln_supsensor_cmd_t * clone = malloc(sizeof(player_smrtln_supsensor_cmd_t));
  if (clone)
    player_smrtln_supsensor_cmd_t_copy(clone,msg);
  return clone;
}
void player_smrtln_supsensor_cmd_t_free(player_smrtln_supsensor_cmd_t *msg)
{      
  player_smrtln_supsensor_cmd_t_cleanup(msg);
  free(msg);
}
unsigned int player_smrtln_supsensor_cmd_t_sizeof(player_smrtln_supsensor_cmd_t *msg)
{
  return sizeof(player_smrtln_supsensor_cmd_t);
} 

int xdr_player_pathplanner_test_one_cmd_t (XDR* xdrs, player_pathplanner_test_one_cmd_t * msg)
{   if(xdr_int(xdrs,&msg->value) != 1)
    return(0);
  return(1);
}
int 
player_pathplanner_test_one_cmd_pack(void* buf, size_t buflen, player_pathplanner_test_one_cmd_t * msg, int op)
{
  XDR xdrs;
  int len;
  if(!buflen)
    return 0;
  xdrmem_create(&xdrs, buf, buflen, op);
  if(xdr_player_pathplanner_test_one_cmd_t(&xdrs,msg) != 1)
    return(-1);
  if(op == PLAYERXDR_ENCODE)
    len = xdr_getpos(&xdrs);
  else
    len = sizeof(player_pathplanner_test_one_cmd_t);
  xdr_destroy(&xdrs);
  return(len);
} 
unsigned int player_pathplanner_test_one_cmd_t_copy(player_pathplanner_test_one_cmd_t *dest, const player_pathplanner_test_one_cmd_t *src)
{
  if (dest == NULL || src == NULL)
    return 0;
  memcpy(dest,src,sizeof(player_pathplanner_test_one_cmd_t));
  return sizeof(player_pathplanner_test_one_cmd_t);
} 
void player_pathplanner_test_one_cmd_t_cleanup(const player_pathplanner_test_one_cmd_t *msg)
{
} 
player_pathplanner_test_one_cmd_t * player_pathplanner_test_one_cmd_t_clone(const player_pathplanner_test_one_cmd_t *msg)
{      
  player_pathplanner_test_one_cmd_t * clone = malloc(sizeof(player_pathplanner_test_one_cmd_t));
  if (clone)
    player_pathplanner_test_one_cmd_t_copy(clone,msg);
  return clone;
}
void player_pathplanner_test_one_cmd_t_free(player_pathplanner_test_one_cmd_t *msg)
{      
  player_pathplanner_test_one_cmd_t_cleanup(msg);
  free(msg);
}
unsigned int player_pathplanner_test_one_cmd_t_sizeof(player_pathplanner_test_one_cmd_t *msg)
{
  return sizeof(player_pathplanner_test_one_cmd_t);
} 

int xdr_player_pathplanner_test_two_cmd_t (XDR* xdrs, player_pathplanner_test_two_cmd_t * msg)
{   if(xdr_int(xdrs,&msg->value) != 1)
    return(0);
  return(1);
}
int 
player_pathplanner_test_two_cmd_pack(void* buf, size_t buflen, player_pathplanner_test_two_cmd_t * msg, int op)
{
  XDR xdrs;
  int len;
  if(!buflen)
    return 0;
  xdrmem_create(&xdrs, buf, buflen, op);
  if(xdr_player_pathplanner_test_two_cmd_t(&xdrs,msg) != 1)
    return(-1);
  if(op == PLAYERXDR_ENCODE)
    len = xdr_getpos(&xdrs);
  else
    len = sizeof(player_pathplanner_test_two_cmd_t);
  xdr_destroy(&xdrs);
  return(len);
} 
unsigned int player_pathplanner_test_two_cmd_t_copy(player_pathplanner_test_two_cmd_t *dest, const player_pathplanner_test_two_cmd_t *src)
{
  if (dest == NULL || src == NULL)
    return 0;
  memcpy(dest,src,sizeof(player_pathplanner_test_two_cmd_t));
  return sizeof(player_pathplanner_test_two_cmd_t);
} 
void player_pathplanner_test_two_cmd_t_cleanup(const player_pathplanner_test_two_cmd_t *msg)
{
} 
player_pathplanner_test_two_cmd_t * player_pathplanner_test_two_cmd_t_clone(const player_pathplanner_test_two_cmd_t *msg)
{      
  player_pathplanner_test_two_cmd_t * clone = malloc(sizeof(player_pathplanner_test_two_cmd_t));
  if (clone)
    player_pathplanner_test_two_cmd_t_copy(clone,msg);
  return clone;
}
void player_pathplanner_test_two_cmd_t_free(player_pathplanner_test_two_cmd_t *msg)
{      
  player_pathplanner_test_two_cmd_t_cleanup(msg);
  free(msg);
}
unsigned int player_pathplanner_test_two_cmd_t_sizeof(player_pathplanner_test_two_cmd_t *msg)
{
  return sizeof(player_pathplanner_test_two_cmd_t);
} 

int xdr_player_time_time_req_t (XDR* xdrs, player_time_time_req_t * msg)
{   if(xdr_u_long(xdrs,&msg->time) != 1)
    return(0);
  return(1);
}
int 
player_time_time_req_pack(void* buf, size_t buflen, player_time_time_req_t * msg, int op)
{
  XDR xdrs;
  int len;
  if(!buflen)
    return 0;
  xdrmem_create(&xdrs, buf, buflen, op);
  if(xdr_player_time_time_req_t(&xdrs,msg) != 1)
    return(-1);
  if(op == PLAYERXDR_ENCODE)
    len = xdr_getpos(&xdrs);
  else
    len = sizeof(player_time_time_req_t);
  xdr_destroy(&xdrs);
  return(len);
} 
unsigned int player_time_time_req_t_copy(player_time_time_req_t *dest, const player_time_time_req_t *src)
{
  if (dest == NULL || src == NULL)
    return 0;
  memcpy(dest,src,sizeof(player_time_time_req_t));
  return sizeof(player_time_time_req_t);
} 
void player_time_time_req_t_cleanup(const player_time_time_req_t *msg)
{
} 
player_time_time_req_t * player_time_time_req_t_clone(const player_time_time_req_t *msg)
{      
  player_time_time_req_t * clone = malloc(sizeof(player_time_time_req_t));
  if (clone)
    player_time_time_req_t_copy(clone,msg);
  return clone;
}
void player_time_time_req_t_free(player_time_time_req_t *msg)
{      
  player_time_time_req_t_cleanup(msg);
  free(msg);
}
unsigned int player_time_time_req_t_sizeof(player_time_time_req_t *msg)
{
  return sizeof(player_time_time_req_t);
} 
